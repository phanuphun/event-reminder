<?php
    ...
    // ถ้ามี session uid เข้ามาให้ทำงานหมายความว่า มี user login เข้ามาเเล้วเเล้วให้เเสดงข้อมูลจากทั้ง 3 ตารางลงในกล่องกิจกรรม
    if(isset($_SESSION["Uid"])){
        ...
        $sql = "SELECT activity.* ,user.* , category.* FROM activity 
                ...
                WHERE activity.U_id = '$Uid' AND activity.A_state = 'กำลังดำเนินการ' ORDER BY A_id DESC";

        $result = mysqli_query($conn,$sql);
        $order = 1 ;
    }
    //เมื่อรับค่าจาก logout มาจะทำลาย SESSION ที่ส่งมา
...
<body>
    ...
    <!-- Content -->
        <!-- ถ้ามี username เข้ามา ให้เเส้งช่องค้นหา -->
        <?php if(isset($_SESSION["username"])){ 
            // โดยนำ category มาใช้ในการค้นหาจากตาราง category ใช้ uid อ้างอิงที่ส่งมาจาก session
        ...
        <!-- ส่วนแสดงกล่องกิจกรรมที่ถูกสร้างขึ้นมา-->
        <table class="table table-dark table-striped table-hover ">
            <tbody class="table-light">
                <tr>
                    <div class="container" style="max-width: 90%;">
                        <div class="my-5 "> <!-- จัด margin -->
                            <div class="row row-cols-1 row-cols-md-5 g-5"> <!-- จัดการแถว คอลัมน์ -->
